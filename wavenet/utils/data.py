...
def read_audio(filename, int_values = False, offset = 0, duration = 0):
...
def load_audio(filename, target_sample_rate, trim=True, trim_frame_length=2048):
    audio, sample_rate = read_audio(filename)
    
    audio = librosa.core.resample(audio, sample_rate, target_sample_rate)
...
class Dataset(data.Dataset):
    def __init__(self, data_dir, sample_rate=4000, in_channels=256, trim=True, sample_length = 2):
        ...
        self.filenames = [x for x in sorted(os.listdir(data_dir)) if '.wav' in x]

    def __getitem__(self, index):
        filepath = os.path.join(self.root_path, self.filenames[index])

        raw_audio, _ = load_audio(filepath, self.sample_rate, self.trim)

        max_len = min(raw_audio.shape[0], self.sample_length*_)
        offset = random.randint(0, raw_audio.shape[0] - max_len)

        raw_audio = raw_audio[offset:offset+max_len]
...
class DataLoader(data.DataLoader):
    def __init__(self, data_dir, receptive_fields,
                 sample_size=0, sample_rate=8000, in_channels=256,
